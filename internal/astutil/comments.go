...
func HasNoLintComment(fset *token.FileSet, file *ast.File, pos token.Pos, linter string) bool {
	...
	if !IsNoLintComment(comment, linter) {
		return false // not a //nolint: directive matching linter.
	}

	tokenFile := fset.File(file.FileStart)
	line, commentLine := tokenFile.PositionFor(pos, false).Line, tokenFile.PositionFor(comment.Pos(), false).Line

	return commentLine == line // on this line?
}
